    ...
    this.productService.getProductDetail(this.productId()!).subscribe({
      next: (res) => {
        ...
        if (data && data.product) {
          data.product.imageUrl = data.product.image ? (data.product.image.startsWith('http') ? data.product.image : `http://localhost:8080/resources/images/product/${data.product.image}`) : 'assets/img/default-court.png';
